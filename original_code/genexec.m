...
% initialize:
ATTR=11;
ants=zeros(ATTR+NTotalStates,NAnts);
...
ants(SAMEP,:)=sameplac(ants,food,MASKSIZE);
ants(AGE,:)=ones(1,NAnts);

% the remaining pieces of the ant's state are the control poicies.
ants(ATTR+1:ATTR+NTotalStates,:)=floor(NActions*rand(NTotalStates,NAnts))+1;
for k=1:NAnts,
